...
TEST_CASE("Default policy: quack:read allows Attach + Scan, denies writes / admin", "[authz][default-policy]") {
...
TEST_CASE("Default policy: quack:write implies quack:read + all data-plane DML", "[authz][default-policy]") {
...
TEST_CASE("Default policy: deny reasons cite the missing scope", "[authz][default-policy][reason]") {
	...
	CHECK(EvaluateDefaultPolicy(reader, Action::Ddl).reason == "default policy never permits administrative actions");
}
