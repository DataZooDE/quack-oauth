...
public:
	...
	std::optional<Response> Post(const PostRequest &r) override {
		last_post = r;
		if (next_responses.empty()) return std::nullopt;
...
TEST_CASE("ParseDeviceAuthorizationResponse: full body",
          "[device-code][parse]") {
...
TEST_CASE("ParseDeviceAuthorizationResponse: missing required fields → nullopt",
          "[device-code][parse][error]") {
	CHECK_FALSE(ParseDeviceAuthorizationResponse(
	                R"({"device_code": "x", "user_code": "y"})").has_value());
	CHECK_FALSE(ParseDeviceAuthorizationResponse(
	                R"({"user_code": "y", "verification_uri": "z"})").has_value());
	CHECK_FALSE(ParseDeviceAuthorizationResponse("not json").has_value());
}

TEST_CASE("ParseDeviceAuthorizationResponse: defaults interval to 5s",
          "[device-code][parse]") {
	const auto r = ParseDeviceAuthorizationResponse(R"({
...
TEST_CASE("ParseDevicePollResponse: 200 with valid body → Success + tokens",
          "[device-code][poll]") {
	const auto r = ParseDevicePollResponse(
	    200, R"({"access_token": "AT", "expires_in": 3600})");
...
TEST_CASE("ParseDevicePollResponse: maps each RFC 8628 error code correctly",
          "[device-code][poll]") {
	CHECK(ParseDevicePollResponse(400, R"({"error": "authorization_pending"})")
	          .outcome == DevicePollOutcome::Pending);
	CHECK(ParseDevicePollResponse(400, R"({"error": "slow_down"})").outcome ==
	      DevicePollOutcome::SlowDown);
	CHECK(ParseDevicePollResponse(400, R"({"error": "access_denied"})").outcome ==
	      DevicePollOutcome::Denied);
	CHECK(ParseDevicePollResponse(400, R"({"error": "expired_token"})").outcome ==
	      DevicePollOutcome::Expired);
	CHECK(ParseDevicePollResponse(400, R"({"error": "invalid_grant"})").outcome ==
	      DevicePollOutcome::Error);
	CHECK(ParseDevicePollResponse(500, "").outcome == DevicePollOutcome::Error);
}

TEST_CASE("ParseDevicePollResponse: 200 with junk body → Error",
...
TEST_CASE("RequestDeviceAuthorization: happy path POSTs client_id + scope",
          "[device-code][http]") {
	FakeHttpClient http;
	http.next_responses.push(IHttpClient::Response{
	    200, R"({"device_code": "DC", "user_code": "UC",
	            "verification_uri": "https://idp.test/d",
	            "expires_in": 600, "interval": 5})"});
	const auto r = RequestDeviceAuthorization(
	    http, "https://idp.test/device", "the-client", "", "openid");
	REQUIRE(r.has_value());
...
TEST_CASE("RequestDeviceAuthorization: confidential client adds Basic auth",
          "[device-code][http]") {
	FakeHttpClient http;
	http.next_responses.push(IHttpClient::Response{
	    200, R"({"device_code": "DC", "user_code": "UC",
...
TEST_CASE("PollDeviceTokenEndpoint: body shape (urn grant + device_code)",
          "[device-code][http]") {
	FakeHttpClient http;
	http.next_responses.push(IHttpClient::Response{
	    400, R"({"error": "authorization_pending"})"});
	const auto r = PollDeviceTokenEndpoint(http, "https://idp.test/token",
	                                       "c", "", "DC-xyz");
	CHECK(r.outcome == DevicePollOutcome::Pending);
	REQUIRE(http.last_post.has_value());
	CHECK(http.last_post->body.find(
...
TEST_CASE("PollDeviceTokenEndpoint: simulated full polling loop",
          "[device-code][http]") {
	// Simulates: pending → pending → slow_down → success.
	FakeHttpClient http;
	http.next_responses.push(
	    IHttpClient::Response{400, R"({"error": "authorization_pending"})"});
	http.next_responses.push(
	    IHttpClient::Response{400, R"({"error": "authorization_pending"})"});
	http.next_responses.push(
	    IHttpClient::Response{400, R"({"error": "slow_down"})"});
	http.next_responses.push(IHttpClient::Response{
	    200, R"({"access_token": "AT", "refresh_token": "RT", "expires_in": 3600})"});

	auto step = [&]() {
		return PollDeviceTokenEndpoint(http, "https://idp.test/token", "c", "",
		                               "DC");
	};
...
TEST_CASE("RequestDeviceAuthorization: empty endpoint / client_id short-circuit",
          "[device-code][http]") {
	FakeHttpClient http;
	CHECK_FALSE(RequestDeviceAuthorization(http, "", "c", "", "").has_value());
	CHECK_FALSE(
	    RequestDeviceAuthorization(http, "https://idp.test/d", "", "", "")
	        .has_value());
