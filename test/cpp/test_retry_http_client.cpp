...
struct SleepRecorder {
	...
	void operator()(std::chrono::milliseconds d) { sleeps.push_back(d); }
};
...
TEST_CASE("RetryingHttpClient: no retry on 200", "[retry][http]") {
	...
	SleepRecorder slept;
	RetryingHttpClient client(inner, /*max_retries=*/1,
	                          std::chrono::milliseconds(1000), std::ref(slept));
...
TEST_CASE("RetryingHttpClient: retries once on 5xx, sleeps 1s, succeeds",
          "[retry][http]") {
	ScriptedHttpClient inner;
	inner.get_script = {
	...
	SleepRecorder slept;
	RetryingHttpClient client(inner, 1, std::chrono::milliseconds(1000),
	                          std::ref(slept));
	const auto r = client.Get("https://x");
	REQUIRE(r.has_value());
	CHECK(r->status_code == 200);
...
TEST_CASE("RetryingHttpClient: retries on transport failure (nullopt)",
          "[retry][http]") {
	ScriptedHttpClient inner;
...
TEST_CASE("RetryingHttpClient: does NOT retry on 4xx (client error)",
          "[retry][http]") {
	ScriptedHttpClient inner;
...
TEST_CASE("RetryingHttpClient: after exhausting retries, returns last result",
          "[retry][http]") {
	ScriptedHttpClient inner;
	inner.get_script = {
...
TEST_CASE("RetryingHttpClient: with max_retries=2 honours doubling backoff",
          "[retry][http]") {
	ScriptedHttpClient inner;
	inner.get_script = {
	    std::nullopt,
	...
	SleepRecorder slept;
	RetryingHttpClient client(inner, /*max_retries=*/2,
	                          std::chrono::milliseconds(1000), std::ref(slept));
	const auto r = client.Get("https://x");
	REQUIRE(r.has_value());
	CHECK(r->status_code == 200);
...
TEST_CASE("RetryingHttpClient: Post path uses the same policy",
          "[retry][http]") {
	ScriptedHttpClient inner;
	inner.post_script = {
