...
TEST_CASE("ParseTokenResponse: missing access_token is malformed", "[token-endpoint][parse][error]") {
	CHECK_FALSE(ParseTokenResponse(R"({"token_type": "Bearer"})").has_value());
	CHECK_FALSE(ParseTokenResponse(R"({"access_token": ""})").has_value());
}

TEST_CASE("ParseTokenResponse: error responses are nullopt", "[token-endpoint][parse][error]") {
...
TEST_CASE("AcquireTokenClientCredentials: happy path POSTs form body + Basic auth", "[token-endpoint][http]") {
	FakeHttpClient http;
	http.next_post_response =
	    IHttpClient::Response {200, R"({"access_token": "the-token", "token_type": "Bearer", "expires_in": 600})"};
	const auto r = AcquireTokenClientCredentials(http, "https://idp.test/token", "client-1", "secret-1", "api:read");
	REQUIRE(r.has_value());
...
TEST_CASE("AcquireTokenClientCredentials: empty scope is omitted from body", "[token-endpoint][http]") {
	FakeHttpClient http;
	http.next_post_response = IHttpClient::Response {200, R"({"access_token": "t"})"};
	AcquireTokenClientCredentials(http, "https://idp.test/token", "c", "s", "");
	REQUIRE(http.last_post.has_value());
	CHECK(http.last_post->body == "grant_type=client_credentials");
}

TEST_CASE("AcquireTokenClientCredentials: transport error → nullopt", "[token-endpoint][http][error]") {
	FakeHttpClient http;
	http.next_post_response = std::nullopt;
	CHECK_FALSE(AcquireTokenClientCredentials(http, "https://idp.test/x", "c", "s", "").has_value());
}

TEST_CASE("AcquireTokenClientCredentials: 401 → nullopt", "[token-endpoint][http][error]") {
	FakeHttpClient http;
	http.next_post_response = IHttpClient::Response {401, R"({"error": "invalid_client"})"};
	CHECK_FALSE(AcquireTokenClientCredentials(http, "https://idp.test/x", "c", "s", "").has_value());
}

TEST_CASE("AcquireTokenClientCredentials: empty endpoint / client_id short-circuit", "[token-endpoint][http]") {
	FakeHttpClient http;
	CHECK_FALSE(AcquireTokenClientCredentials(http, "", "c", "s", "").has_value());
	CHECK_FALSE(AcquireTokenClientCredentials(http, "https://idp.test/x", "", "s", "").has_value());
	CHECK_FALSE(http.last_post.has_value());
}

TEST_CASE("AcquireTokenRefreshToken: happy path posts grant_type=refresh_token", "[token-endpoint][refresh]") {
	FakeHttpClient http;
	http.next_post_response = IHttpClient::Response {
	    200, R"({"access_token": "new-at", "refresh_token": "new-rt", "expires_in": 600, "token_type": "Bearer"})"};
	const auto r = AcquireTokenRefreshToken(http, "https://idp.test/token", "c", "s", "old-refresh-token", "");
	REQUIRE(r.has_value());
...
TEST_CASE("AcquireTokenRefreshToken: public client (no secret) omits Basic auth", "[token-endpoint][refresh]") {
	FakeHttpClient http;
	http.next_post_response = IHttpClient::Response {200, R"({"access_token": "x", "token_type": "Bearer"})"};
...
TEST_CASE("AcquireTokenRefreshToken: scope is optional and URL-encoded", "[token-endpoint][refresh]") {
	FakeHttpClient http;
	http.next_post_response = IHttpClient::Response {200, R"({"access_token": "x"})"};
	AcquireTokenRefreshToken(http, "https://idp.test/x", "c", "s", "rt", "openid api:read");
	REQUIRE(http.last_post.has_value());
	CHECK(http.last_post->body.find("scope=openid%20api%3Aread") != std::string::npos);
}

TEST_CASE("AcquireTokenRefreshToken: empty endpoint / refresh_token short-circuit", "[token-endpoint][refresh]") {
	FakeHttpClient http;
	CHECK_FALSE(AcquireTokenRefreshToken(http, "", "c", "s", "rt", "").has_value());
	CHECK_FALSE(AcquireTokenRefreshToken(http, "https://idp.test/x", "c", "s", "", "").has_value());
	CHECK_FALSE(http.last_post.has_value());
}
...
TEST_CASE("AcquireTokenRefreshToken: non-200 → nullopt", "[token-endpoint][refresh]") {
	FakeHttpClient http;
	http.next_post_response =
	    IHttpClient::Response {400, R"({"error": "invalid_grant", "error_description": "Token is not active"})"};
	CHECK_FALSE(AcquireTokenRefreshToken(http, "https://idp.test/x", "c", "s", "rt", "").has_value());
}

TEST_CASE("AcquireTokenRefreshToken: response that omits a new refresh_token works", "[token-endpoint][refresh]") {
	// Some IdPs don't rotate refresh_tokens. The caller keeps using the
	// original one in that case.
	FakeHttpClient http;
	http.next_post_response = IHttpClient::Response {200, R"({"access_token": "new-at", "expires_in": 600})"};
	const auto r = AcquireTokenRefreshToken(http, "https://idp.test/x", "c", "s", "rt", "");
	REQUIRE(r.has_value());
	CHECK(r->access_token == "new-at");
