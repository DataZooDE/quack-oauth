...
TEST_CASE("ParseIntrospectionResponse: active=true with full claim set",
          "[introspect][parse]") {
...
TEST_CASE("ParseIntrospectionResponse: active=false stays parseable",
          "[introspect][parse]") {
...
TEST_CASE("ParseIntrospectionResponse: array-shaped audience (Entra)",
          "[introspect][parse][aud]") {
	const auto r = ParseIntrospectionResponse(R"({
	  "active": true,
...
TEST_CASE("ParseIntrospectionResponse: scp[] array (Entra)",
          "[introspect][parse][scope]") {
	const auto r = ParseIntrospectionResponse(R"({
	  "active": true,
...
TEST_CASE("ParseIntrospectionResponse: missing `active` is malformed",
          "[introspect][parse][error]") {
...
TEST_CASE("ParseIntrospectionResponse: malformed JSON / empty",
          "[introspect][parse][error]") {
...
TEST_CASE("IntrospectToken: happy path POSTs with form body + Basic auth",
          "[introspect][http]") {
	FakeHttpClient http;
	http.next_post_response = IHttpClient::Response{
	    200, R"({"active": true, "sub": "alice"})"};

	const auto r = IntrospectToken(http, "https://idp.test/introspect",
	                               "quack-client", "shh", "raw.token.value");
	REQUIRE(r.has_value());
	CHECK(r->active);
	CHECK(r->subject == "alice");
...
TEST_CASE("IntrospectToken: URL-encodes tokens that contain reserved chars",
          "[introspect][http]") {
	FakeHttpClient http;
	http.next_post_response = IHttpClient::Response{
	    200, R"({"active": false})"};

	IntrospectToken(http, "https://idp.test/introspect", "c", "s",
	                "weird+token/with=chars");
	REQUIRE(http.last_post.has_value());
	// `+`, `/`, `=` MUST be percent-encoded -- form values aren't URL-safe.
	CHECK(http.last_post->body.find("weird%2Btoken%2Fwith%3Dchars") !=
	      std::string::npos);
}

TEST_CASE("IntrospectToken: transport error returns nullopt",
          "[introspect][http][error]") {
	FakeHttpClient http;
	http.next_post_response = std::nullopt;
	CHECK_FALSE(IntrospectToken(http, "https://idp.test/x", "c", "s", "t").has_value());
}

TEST_CASE("IntrospectToken: non-200 HTTP returns nullopt",
          "[introspect][http][error]") {
	FakeHttpClient http;
...
TEST_CASE("IntrospectToken: 200 with malformed body returns nullopt",
          "[introspect][http][error]") {
	FakeHttpClient http;
...
TEST_CASE("IntrospectToken: empty endpoint or token short-circuits without HTTP",
          "[introspect][http]") {
	FakeHttpClient http;
...
TEST_CASE("IntrospectToken: active=false is surfaced as a populated Response",
          "[introspect][http]") {
