...
TEST_CASE("ProviderFromString recognises canonical names",
          "[providers][parse]") {
...
TEST_CASE("ProviderFromString defaults unknown names to Generic",
          "[providers][parse]") {
...
TEST_CASE("Entra: tenant substitutes into issuer and JWKS URI",
          "[providers][entra]") {
	const auto r = ResolveProvider(ProviderId::Entra,
...
TEST_CASE("Entra: `common` and `organizations` are accepted as tenant placeholders",
          "[providers][entra]") {
...
TEST_CASE("Google: tenant is ignored; validation is Tokeninfo",
          "[providers][google]") {
...
TEST_CASE("Keycloak: the realm-URL prefix substitutes into all three URIs",
          "[providers][keycloak]") {
	const auto r = ResolveProvider(ProviderId::Keycloak,
	                               "http://localhost:8080/realms/main");
	CHECK(r.validation == ProviderValidation::Jwks);
	CHECK(r.issuer == "http://localhost:8080/realms/main");
	CHECK(r.jwks_uri ==
	      "http://localhost:8080/realms/main/protocol/openid-connect/certs");
	CHECK(r.introspection_endpoint ==
	      "http://localhost:8080/realms/main/protocol/openid-connect/token/introspect");
}

TEST_CASE("Generic: all materialised URIs are empty -- operator must supply",
          "[providers][generic]") {
