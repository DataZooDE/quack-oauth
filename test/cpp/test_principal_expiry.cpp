...
TEST_CASE("IsPrincipalExpired: now before exp → not expired", "[principal-expiry]") {
	const auto p = MakeP(1700001000);
	CHECK_FALSE(IsPrincipalExpired(p, 1700000000, /*skew=*/0));
}

TEST_CASE("IsPrincipalExpired: now equals exp → not expired (exp is exclusive)", "[principal-expiry]") {
	const auto p = MakeP(1700001000);
	CHECK_FALSE(IsPrincipalExpired(p, 1700001000, 0));
}

TEST_CASE("IsPrincipalExpired: now after exp → expired", "[principal-expiry]") {
	const auto p = MakeP(1700001000);
	CHECK(IsPrincipalExpired(p, 1700001001, 0));
	CHECK(IsPrincipalExpired(p, 1700009999, 0));
}

TEST_CASE("IsPrincipalExpired: clock-skew gives leeway past exp", "[principal-expiry]") {
	const auto p = MakeP(1700001000);
...
TEST_CASE("IsPrincipalExpired: exp=0 means 'unknown' -- treat as not expired", "[principal-expiry]") {
...
TEST_CASE("IsPrincipalExpired: negative exp treated like 0 (unknown)", "[principal-expiry]") {
