...
| **S-14 Wasm gating (source-side)** ✅ | `DUCKDB_NATIVE_ONLY_SOURCES` (http_client_duckdb, check_token, login, refresh, device_login) is conditionally excluded in `CMakeLists.txt` when `EMSCRIPTEN` is set. The matching `Register*` calls in `quack_oauth_extension.cpp` are guarded by `#ifndef __EMSCRIPTEN__`. Wasm build retains settings, SECRET types, `diagnose()`, and `quack_oauth_check_authorization` (PURE_SOURCES + state are all wasm-safe; `policy_table.cpp` + `sql_inspect.cpp` use only DuckDB's own `Connection` / `Parser` APIs, no native deps). PURE_SOURCES (JWT verify/parse, JWKS cache, decision cache, providers, authz, policy, redact, tokeninfo/introspect parsers, token_endpoint parser, device_code parser) compile under emscripten unchanged. **Wasm is excluded from CI (issue #3):** the loadable side-module (`emcc -sSIDE_MODULE=2`) links only libraries named in `duckdb_extension_load(... LINKED_LIBS ...)`; with none declared, jwt-cpp's OpenSSL symbols are unresolved and the `.wasm` *builds* green but fails to **load** in the browser. The source split is kept for a future `LINKED_LIBS`-based re-enable (see CLAUDE.md "Wasm path"). | Native build, Catch2 (786 assertions), SQL (162), Keycloak integration (7 tests) all green; wasm gating verified by `make ci_config_test` (`scripts/test_ci_wasm_excluded.sh`) — the three wasm archs are absent from the generated matrix |
...
- **S-14 wasm CI matrix** — **deferred / excluded (issue #3).** The
  source-side split is in place (see S-14 row above), but the wasm archs
  are excluded in `.github/workflows/MainDistributionPipeline.yml`
  (`exclude_archs: …;wasm_mvp;wasm_eh;wasm_threads`) because the side-module
  fails to *load* without OpenSSL linked via `LINKED_LIBS` — a failure the
  build step can't catch. Re-enabling means: declare the vcpkg
  wasm32-emscripten OpenSSL archives in `LINKED_LIBS` on the
  `duckdb_extension_load(quack_oauth ...)` call, drop the wasm archs from
  `exclude_archs`, and prove it with a real duckdb-wasm browser/node load
  (not just CI build-green).
