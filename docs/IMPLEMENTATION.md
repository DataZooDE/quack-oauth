...
| **S-13a Default policy (pure)** ✅ | R-S-8 default policy: quack:read → Attach + Scan; quack:write → also CopyTo/CopyFrom (and implies read); ServeAdmin always denied. Action enum, PolicyOutcome with reason for diagnostics. Object glob filtering reserved for a future schema extension. | Catch2: `test_authz.cpp` (6 cases, 23 assertions) |
...
| **S-13c action detection** ✅ | `DetectAction(query_string)` classifies SQL: ATTACH → Attach, COPY ... TO → CopyTo, COPY ... FROM → CopyFrom, PRAGMA quack_serve|quack_stop|quack_restart → ServeAdmin, everything else → Scan. Strips leading whitespace + `--` line comments + `/* ... */` block comments. Wired into `quack_oauth_check_authorization` -- the hardcoded Scan is gone. | Catch2: `test_action_detect.cpp` (6 cases, 26 assertions); live test verifies each action shape against Keycloak |
| **S-13d SQL-native policy** ✅ | `src/policy.{hpp,cpp}` (PURE_SOURCES) hosts the rule + evaluator types; `src/policy_table.{hpp,cpp}` (DUCKDB_WASM_SAFE_SOURCES) loads rules from a SQL table via a short-lived `Connection`. Schema: `priority INTEGER NOT NULL, subject VARCHAR, any_scope VARCHAR[], actions VARCHAR[], allow BOOLEAN NOT NULL`. Rules sorted by ascending priority; first match wins. `policy_table` field on the server SECRET names the table; `quack_oauth_policy_default` setting (`allow`|`deny`, default `deny`) controls the no-match fallback. **Fail-closed**: if `policy_table` is set but the query fails (missing table, wrong schema, invalid action name), denies regardless of principal. Falls back to the default scope policy when `policy_table` is unset. **No yaml-cpp dependency** -- the parser the YAML iteration of this slice required is gone. | Catch2: `test_policy.cpp` (9 cases, evaluator semantics); SQL: `oauth_policy_table_keycloak.test` (creates the policy table inline + live ROPC token; verifies scope-gated allow, action-gated deny, setting-driven default flip, unknown-session deny) |
...
| **S-14 Wasm gating (source-side)** ✅ | `DUCKDB_NATIVE_ONLY_SOURCES` (http_client_duckdb, check_token, login, refresh, device_login) is conditionally excluded in `CMakeLists.txt` when `EMSCRIPTEN` is set. The matching `Register*` calls in `quack_oauth_extension.cpp` are guarded by `#ifndef EMSCRIPTEN`. Wasm build retains settings, SECRET types, `diagnose()`, and `quack_oauth_check_authorization` (PURE_SOURCES + state are all wasm-safe; `policy_table.cpp` uses only DuckDB's own `Connection` API, no native deps). PURE_SOURCES (JWT verify/parse, JWKS cache, decision cache, providers, authz, policy, action_detect, redact, tokeninfo/introspect parsers, token_endpoint parser, device_code parser) compile under emscripten unchanged. Live emcc build still requires the EMSDK toolchain in CI (`make wasm_mvp` via extension-ci-tools) -- gated separately. | Native build, Catch2 (544 assertions), SQL (89), Keycloak integration (7 tests) all green after the split; CMake configure path with `-DEMSCRIPTEN=1` parses to the smaller source set |
