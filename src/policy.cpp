...
static bool PrincipalHasAnyScope(const Principal &p, const std::vector<std::string> &any_of) {
	if (any_of.empty())
...
static bool ActionMatches(Action action, const std::vector<Action> &list) {
	if (list.empty())
...
PolicyOutcome EvaluatePolicy(const PolicyDocument &doc, const Principal &principal, Action action,
	...
	for (const auto &rule : doc.rules) {
		...
		return {rule.allow ? Decision::Allow : Decision::Deny, rule.allow ? "rule allow" : "rule deny"};
	}
	return {doc.default_allow ? Decision::Allow : Decision::Deny, doc.default_allow ? "default allow" : "default deny"};
}
