...
VerifyResult ValidateTokenViaGithubCheck(std::string_view token,
	...
	if (token.empty()) return VerifyResult::Malformed;
	...
	if (!resp.has_value()) return VerifyResult::JwksFetchFailed;
	if (resp->status_code >= 500) return VerifyResult::JwksFetchFailed;
	if (resp->status_code == 404) return VerifyResult::InvalidSignature;
	if (resp->status_code != 200) return VerifyResult::InvalidSignature;

	const auto principal = ParseGithubCheckResponse(resp->body);
	if (!principal.has_value()) return VerifyResult::Malformed;
	if (out_principal != nullptr) *out_principal = *principal;
