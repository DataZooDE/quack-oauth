...
static void EmitUserNotice(const quack_oauth::DeviceAuthorizationResponse &auth) {
	...
	std::cerr << "[quack_oauth_device_login] visit "
	          << (auth.verification_uri_complete.empty() ? auth.verification_uri : auth.verification_uri_complete)
...
string DoDeviceLoginImpl(ClientContext &context, const string &secret_name) {
	...
	if (device_endpoint.empty()) {
		throw InvalidInputException("quack_oauth_device_login: SECRET '%s' is missing "
		                            "`device_authorization_endpoint`",
		                            secret_name);
	}
	if (token_endpoint.empty() || client_id.empty()) {
		throw InvalidInputException("quack_oauth_device_login: SECRET '%s' is missing `token_endpoint` "
		                            "or `client_id`",
		                            secret_name);
	}
	...
	const auto auth = quack_oauth::RequestDeviceAuthorization(http, device_endpoint, client_id, client_secret, scope);
	if (!auth.has_value()) {
		throw IOException("quack_oauth_device_login: device_authorization_endpoint '%s' did "
	...
	int64_t interval = auth->interval > 0 ? auth->interval : 5;
	const auto start = std::chrono::steady_clock::now();
	const auto deadline = start + std::chrono::seconds(auth->expires_in > 0 ? auth->expires_in : 600);
	while (std::chrono::steady_clock::now() < deadline) {
		std::this_thread::sleep_for(std::chrono::seconds(interval));
		const auto poll =
		    quack_oauth::PollDeviceTokenEndpoint(http, token_endpoint, client_id, client_secret, auth->device_code);
		switch (poll.outcome) {
		...
		case quack_oauth::DevicePollOutcome::Success: {
			if (!poll.tokens.has_value()) {
				throw IOException("quack_oauth_device_login: token endpoint returned success "
				                  "but no access_token");
			}
			const auto now =
			    std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch())
			        .count();
			const auto expires_at_iso = FormatUtcIso8601(now + poll.tokens->expires_in);
			PersistTokenFields(context, accessor, poll.tokens->access_token, expires_at_iso,
			                   poll.tokens->refresh_token);
			return expires_at_iso;
		}
		case quack_oauth::DevicePollOutcome::Denied:
			throw IOException("quack_oauth_device_login: user denied the authorization request");
		case quack_oauth::DevicePollOutcome::Expired:
			throw IOException("quack_oauth_device_login: device_code expired before the user "
			                  "completed the authorization flow");
		case quack_oauth::DevicePollOutcome::Error:
		default:
			throw IOException("quack_oauth_device_login: token endpoint returned an "
			                  "unexpected error while polling");
		}
	}
	throw IOException("quack_oauth_device_login: polling timed out before user authorized");
}
