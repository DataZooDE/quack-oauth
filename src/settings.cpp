...
Value EnvStringDefault(const char *env_name, const char *fallback) {
	const auto v = quack_oauth::EnvString(env_name);
	return v.empty() ? Value(fallback) : Value(v);
}

Value EnvBoolDefault(const char *env_name, bool fallback) {
	return Value::BOOLEAN(quack_oauth::EnvBoolOrDefault(env_name, fallback));
}

Value EnvIntDefault(const char *env_name, std::int32_t fallback) {
...
void RegisterQuackOauthSettings(DBConfig &config) {
	...
	config.AddExtensionOption(
	    "quack_oauth_enabled",
	    "Swap quack's auth callbacks for the OAuth implementation (R-S-1).",
	    LogicalType::BOOLEAN,
	...
	config.AddExtensionOption(
	    ...
	    "Token validation strategy: 'jwks' or 'introspect' (R-S-2).",
	    LogicalType::VARCHAR,
	...
	config.AddExtensionOption(
	    "quack_oauth_provider",
	    "First-class IdP preset: entra|google|keycloak|okta|github|generic (R-S-12).",
	    LogicalType::VARCHAR,
	    EnvStringDefault("QUACK_OAUTH_PROVIDER", "generic"), nullptr, SetScope::GLOBAL);

	// R-S-3: clock skew for JWT exp/nbf/iat checks.
	config.AddExtensionOption(
	    "quack_oauth_clock_skew_s",
	    "Allowable clock skew (seconds) when verifying JWT exp/nbf/iat (R-S-3).",
	    LogicalType::INTEGER,
	    EnvIntDefault("QUACK_OAUTH_CLOCK_SKEW_S", 60), nullptr, SetScope::GLOBAL);

	// R-S-4: rate-limit per-kid JWKS refresh to guard against poll DoS.
	config.AddExtensionOption(
	    "quack_oauth_jwks_min_refresh_s",
	    "Minimum seconds between JWKS refreshes per kid (R-S-4).",
	    LogicalType::INTEGER,
	    EnvIntDefault("QUACK_OAUTH_JWKS_MIN_REFRESH_S", 30), nullptr, SetScope::GLOBAL);

	// R-S-5: cache RFC 7662 introspect results.
	config.AddExtensionOption(
	    "quack_oauth_introspect_cache_s",
	    "Cache lifetime (seconds) for introspect-mode decisions, capped at token exp (R-S-5).",
	    LogicalType::INTEGER,
	    EnvIntDefault("QUACK_OAUTH_INTROSPECT_CACHE_S", 30), nullptr, SetScope::GLOBAL);

	// R-C-2: client-side proactive renewal window.
	config.AddExtensionOption(
	    "quack_oauth_renew_skew_s",
	    "Client refreshes the access token this many seconds before expires_at (R-C-2).",
	    LogicalType::INTEGER,
	    EnvIntDefault("QUACK_OAUTH_RENEW_SKEW_S", 60), nullptr, SetScope::GLOBAL);
	...
	config.AddExtensionOption(
	    "quack_oauth_policy_default",
	    "Default decision when no policy_table rule matches: 'allow' or 'deny' (R-S-7).",
	    LogicalType::VARCHAR,
	    EnvStringDefault("QUACK_OAUTH_POLICY_DEFAULT", "deny"), nullptr, SetScope::GLOBAL);

	// R-N-4: explicit operator opt-in to serving bearer tokens over plaintext.
	config.AddExtensionOption(
	    "quack_oauth_trust_plaintext",
	    "Allow LOAD with enabled=true even when no TLS terminator is detected (R-N-4).",
	    LogicalType::BOOLEAN,
	    EnvBoolDefault("QUACK_OAUTH_TRUST_PLAINTEXT", false), nullptr, SetScope::GLOBAL);
	...
	config.AddExtensionOption(
	    ...
	    "Name of the quack_oauth_server SECRET that check_token reads.",
	    LogicalType::VARCHAR,
