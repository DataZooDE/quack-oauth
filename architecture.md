...
| ADR-4 | Policy as a SQL table in the server's DuckDB database, addressed by `policy_table` on the server SECRET. Rules carry `(priority, subject, any_scope, actions, object_pattern, column_pattern, allow)` columns; `object_pattern` + `column_pattern` are optional and load as NULL (match-any) for tables predating their addition. The action and the touched objects/columns are derived by parsing the incoming SQL with DuckDB's own parser (`duckdb::Parser`). | YAML file on disk; Rego/OPA; a plug-in evaluator API. | The host is already a DuckDB process — reusing its query engine eliminates an external parser (no yaml-cpp dep), lets operators manage rules with `INSERT`/`UPDATE`/`DELETE`, joins to existing principal tables, transactions, etc. Reusing the parser for action detection scales the policy from coarse verb-only to object- and column-level without re-implementing SQL parsing. The default scope policy (R-S-8) still applies when no table is configured. |
...
|---|---|---|
| **IdP key rotation** | IdP rotates signing key; old JWKS cached. | First request with new `kid` (or signature failure on same `kid`) triggers a rate-limited JWKS fetch (bounded by `min_refresh_s`); subsequent requests hit cache. Fresh IdP key material commits to cache even under unverified presenting tokens. Keys removed by the IdP are revoked on successful 200 refresh. |
...
| Row-level predicates beyond the SQL-table policy's object + column gating. | Medium | Low | Operators express row-level filtering via DuckDB views that pre-filter; the policy then targets the view's object name. For deeper requirements an OPA/Cedar evaluator can swap behind the same `quack_oauth_check_authorization` callback. Object + column gating itself landed in the parser-driven AuthZ refresh (commit 6f1b649). |
